...
export function getVisibleEvents(
  ...
  includeArchivedPast = false,
) {
  const firstVisibleDate = hidePastEvents ? todayIso() : includeArchivedPast ? undefined : daysAgoIso(archiveLookbackDays);

  return events
    .filter((event) => !firstVisibleDate || event.date >= firstVisibleDate)
